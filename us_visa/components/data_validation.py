...
class DataValidation:
    ...
    def initiate_data_validation(self):
        try:
            validation_err_msg=''
            train_df,test_df=(self.read_data(self.data_ingestion_artifact.trained_file_path),self.read_data(self.data_ingestion_artifact.test_file_path))
            status=self.validate_no_of_columns(train_df)
            logging.info(f"All required columns present in training dataframe: {status}")
            if not status:
                validation_err_msg +=f"Columns are missing in traiinig dataframe"
